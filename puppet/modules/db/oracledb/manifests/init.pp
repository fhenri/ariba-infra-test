class oracle::server (
  ...
  file {
    ...
    "$ORACLE_ROOT/tmp/database":
      ...
      source  => '/vagrant/puppet/modules/db/oracledb/files/database',
      recurse => 'remote',
      owner   => "$ORACLE_USER",
      group   => "$DBA_GROUP", 
      require => Group["$DBA_GROUP"],
    ...
    "$ORACLE_HOME/ctx/data/":
      ...
      source  => "/vagrant/puppet/modules/db/oracledb/files/examples/ctx/data",
      recurse => 'remote',
      purge   => true,
      replace => "no",
      owner   => "$ORACLE_USER",
  ...
  exec {
    "kernelprops":
      command => "/vagrant/puppet/modules/db/oracledb/files/CVU_11.2.0.1.0_vagrant/runfixup.sh",
      user    => root;
